            ...
            <% if(currentUser){ %>
              ...
              <% } else if(hasVote[k] == 0 || hasVote[k] == 1 || hasVote[k] == -1){ %>
                ...
                <% if(0 <= rank && rank <= 2 && posts[k].moderation == 1){ %>
                  <span>
                    <button id="moderation<%= posts[k]._id %>" class="moderation btn btnxxs btn-primary text-sm ml-2 noshadow" name="published" value="0" title="Post moderation" type="submit">Exclusive</button>
                  </span>
                <% } else if(0 <= rank && rank <= 2 && posts[k].moderation == 0){ %>
                  <span>
                    <button id="moderation<%= posts[k]._id %>" class="moderation btn btnxxs btn-info text-sm ml-2 noshadow" name="exclusive" value="1" title="Post moderation" type="submit">Published</button>
                  </span>
                <% } %>
                  <span class="invisible" id="modVisibility<%= posts[k]._id %>"></span>
            <% } else{ %>
