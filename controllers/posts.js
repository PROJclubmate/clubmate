...
router.get('/home-morePosts', function(req, res){
  if(req.user){
    var userClubIds = req.user.userClubs.map(function(club){
      return club.id;
    });
...
// FRIENDS' - show what friends have posted
...
router.get('/friends-morePosts', function(req, res){
  if(req.user){
    if(req.query.ids != ''){
      var seenIds = req.query.ids.split(',');
    } else{
...
router.get('/discover-morePosts', function(req, res){
  if(req.query.ids != ''){
    var seenIds = req.query.ids.split(',');
  } else{
    var seenIds = [];
...
router.post('/clubs/:club_id/posts', middleware.isLoggedIn, upload.single('image'), function(req, res){
  var CU_50_profilePic = cloudinary.url(req.user.profilePicId,
  {width: 50, height: 50, quality: 100, secure: true, crop: 'fill', format: 'jpg'});
  if(req.file){
    if(req.body.privacy){
...
router.get('/clubs/:club_id/posts/:post_id', function (req, res){
...
// QUOTE
router.get('/clubs/:club_id/posts/:post_id/subPost/:bucket_id', function(req, res){
  if(req.query.quote){
    Post.findById(req.params.post_id).populate({path: 'postClub', select: 'name avatar avatarId clubUsers'})
    .exec(function (err, foundPost){
    if(err || !foundPost){
...
// UPDATE POST ROUTE
router.put('/clubs/:club_id/posts/:post_id', middleware.checkPostOwnership, function(req, res){
...
// DESTROY POST ROUTE
router.delete('/clubs/:club_id/posts/:post_id', middleware.checkPostOwnership, function(req, res){
...
//Posts Vote route
router.put('/posts/:post_id/vote', middleware.isLoggedIn, function(req, res){
  Post.findById(req.params.post_id, function(err, foundPost){
  if(err || !foundPost){
...
//Posts modVote route
router.put('/posts/:post_id/modvote', middleware.isLoggedIn, function(req, res){
  Post.findById(req.params.post_id, function(err, foundPost){
  if(err || !foundPost){
