...
module.exports = {
  ...
  chatsListClubRooms(req, res, next){
    ...
    .select({_id: 1, name: 1, avatar: 1, avatarId: 1, conversationId: 1}).exec(function(err, foundClubs){
    if(err){
      ...
      for(var i=0;i<foundClubs.length;i++){
        if(foundClubs[i]._id == req.params.club_id){
          ...
          openedClub['conversationId'] = null;
        }
        if(environment === 'dev'){
      ...
      res.render('chats/index_rooms', {clubs: foundClubs, Clubs_50_clubAvatar, openedClub, conversationId: null,
        convClubId: null, currentUserId: req.user._id, currentUser: req.user, chatHeadName: null, cdn_prefix});
      return User.updateOne({_id: req.user._id}, 
