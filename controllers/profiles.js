                    ...
                    Conversation.findOne({ _id: conversationId }, function (err, foundConversation) {
                      if (err || !foundConversation) {
                        ...
                        res.render('users/show', {
                          haveRequest, sentRequest, isFriend, user: foundUser,
            ...
            .exec(function (err, foundFriends) {
              if (err || !foundUser) {
                ...
                return res.render("users/show", {
                  haveRequest, sentRequest, isFriend, user: foundUser, match, adminClubs,
        ...
        if (err || !foundUser) {
          ...
          if (req.user) {
            ...
            if (isFriend || match) {
              ...
              res.json({
                clubs: limitedClubs, clubCount, Clubs_50_clubAvatar, newEndpoints,
      ...
      if(err || !foundUserPosts){
        ...
        if(!req.user._id.equals(req.params.id)){
          ...
          function(err){
            ...
            res.json({
              hasVote, hasModVote, posts: modPosts, match, currentUser, foundPostIds,
      ...
      if(err || !foundUserPosts){
        ...
        function(err){
          if(err){
            ...
            return res.json({
              hasVote, hasModVote, posts: userPosts, match, currentUser, foundPostIds,
              PC_50_clubAvatar, CU_50_profilePic, arrLength, csrfToken: res.locals.csrfToken, cdn_prefix
            });
          }
        });
      ...
      Post.aggregate([
        ...
        function(err){
          if(err){
            ...
            res.json({
              hasVote, hasModVote, posts: modPosts, match, currentUser, foundHPostIds,
          ...
          if (req.user) {
            ...
            if (0 <= rank && rank <= 4) {
              ...
              Post.find({ postClub: req.params.club_id, topic: { $ne: '' }, createdAt: { $gt: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000) } })
                .select({
                  topic: 1, image: 1, imageId: 1, subpostsCount: 1, upVoteCount: 1, downVoteCount: 1, moderation: 1,
                  postAuthor: 1, postClub: 1
                }).sort({ upVoteCount: -1 }).limit(10).exec(function (err, topTopicPosts) {
                  if (err || !topTopicPosts) {
                    ...
                    User.countDocuments({
                      _id: { $in: clubUserIds },
                      lastActive: { $gt: new Date(Date.now() - 120 * 1000) }
                    }, function (err, onlineClubMembersCount) {
                      res.render('clubs/show', {
                        rank, currentUser: req.user, users: limitedUsers, conversationId, convClubId,
            ...
            return res.render('clubs/show', {
              currentUser, rank, users: limitedUsers, club: foundClub, topTopicPosts,
        ...
        Post.find({ postClub: req.params.club_id, topic: { $ne: '' } })
          .select({
            ...
            if (err || !topTopicPosts) {
              ...
              res.json({
                Posts_50_Image, topTopicPosts: modTopTopicPosts, clubId: req.params.club_id,
        ...
        if (err || !foundClub) {
          ...
          if (req.user) {
            ...
            if (junior) {
              ...
              var newEndpoints = newStart + ',' + newEnd;
              res.json({
                users: limitedUsers, Users_50_profilePic, newEndpoints, clubId: foundClub._id,
          ...
          if (req.user) {
            ...
            if (junior == true) {
              if (req.query.name && req.query.name != '') {
                ...
                res.json({
                  users: matchingUsers, Users_50_profilePic, clubId: foundClub._id, rank,
                  csrfToken: res.locals.csrfToken, cdn_prefix
                });
                return User.updateOne({ _id: req.user._id }, { $currentDate: { lastActive: true } }).exec();
              } else {
      ...
      .exec(function (err, foundClub) {
        if (err || !foundClub) {
          ...
          if (admin) {
            ...
            var newEndpoints = newStart + ',' + newEnd;
            res.json({
              users: limitedUsers, MemberRequests_50_profilePic, newEndpoints, club: foundClub,
              csrfToken: res.locals.csrfToken, cdn_prefix
            });
            return User.updateOne({ _id: req.user._id }, { $currentDate: { lastActive: true } }).exec();
          } else {
      ...
      if(err || !clubPosts){
        ...
        function(err){
          if(err){
            ...
            var rank = currentRank2(req.params.club_id, req.user.userClubs);
            res.json({
              hasVote, hasModVote, posts: modPosts, rank, currentUser, foundPostIds,
      ...
      if(err || !clubPosts){
        ...
        function(err){
          if(err){
            ...
            return res.json({
              hasVote, hasModVote, posts, rank, currentUser, foundPostIds, PA_50_profilePic,
