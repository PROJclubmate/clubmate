...
module.exports = {
  ...
  indexRequests(req, res, next){
    ...
    if(req.body.acceptInvite){
      ...
      if(err || !foundUser){
        ...
        if(err || !foundClub){
          console.log(req.user._id+' => (index-16)foundClub err:- '+JSON.stringify(err, null, 2));
          req.flash('error', 'Something went wrong :(');
          // return res.redirect('back');
        } else{
  ...
  indexViewAllFriends(req, res, next){
    ...
    if(err || !foundUser){
      console.log('(index-30)foundUser err:- '+JSON.stringify(err, null, 2));
      req.flash('error', 'Something went wrong :(');
      return res.redirect('back');
    } else{
      if(foundUser._id.equals(req.user._id) || foundUser.friends.includes(req.user._id)){
        ...
        if(err || !foundFriends){
          console.log('(index-31)foundFriends err:- '+JSON.stringify(err, null, 2));
          req.flash('error', 'Something went wrong :(');
          return res.redirect('back');
        } else{
